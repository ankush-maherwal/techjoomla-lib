...
foreach ($iterator as $file)
{
	if ($file->isFile())
	{
		...
		if ($continue)
		{
			...
			if (preg_match('#2012\s+-\s+[0-9]{4}\s+Techjoomla.\s+All\s+rights#', $fileContents)
				&& !preg_match('#2012\s+-\s+' . $year . '\s+Techjoomla.\s+All\s+rights#', $fileContents))
			{
				$changeCopyrightDate = true;
				$fileContents = preg_replace('#2012\s+-\s+[0-9]{4}\s+Techjoomla.\s+All\s+rights#', '2012 - ' .
